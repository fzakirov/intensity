...
def plot_evoked(ch_name, list_of_evoked_1, evoked_sub_1,
    ...
    plt.plot(ga_evoked_1.times, ga_evoked_array * 10 ** 6, 'b',
             label='TD (n=' + str(len(list_of_evoked_1)) + ') ' + condition)
    plt.plot(ga_evoked_1.times, evoked_sub_1_array * 10 ** 6, 'r', label=sub + ' ' + condition)

    # plt.plot(ga_evoked_array+gstd_evoked_array)
    # plt.plot(ga_evoked_array-gstd_evoked_array)
    plt.fill_between(ga_evoked_1.times, ga_evoked_array * 10 ** 6 - std_num * gstd_evoked_array * 10 ** 6,
                     ga_evoked_array * 10 ** 6 + std_num * gstd_evoked_array * 10 ** 6, 'b', alpha=0.3)
...
'''
makes event-specific TFR from epoch instance

epochs - epoch instance
event - event to make TFR
event_ids - str list of events IDs
...
def event_to_power(epochs, event, event_ids, baseline):
...
'''
makes TFR from epoch instance

epochs - epoch instance
baseline - baseline to use
tmin - tmin to crop TFR object
tmax - tmax to crop TFR object
...
def epoch_to_power(epoch, baseline, tmin, tmax):
    ...
    n_cycles = freqs
    power = tfr_multitaper(epoch.average(), freqs=freqs, n_cycles=n_cycles,
...
'''
makes event-specific TFR from GA instance
...
def ga_to_power(ga, baseline):
    freqs = np.arange(0.2, 36, 0.2)
    n_cycles = freqs
    power = tfr_multitaper(ga, freqs=freqs, n_cycles=n_cycles,
                           use_fft=True, return_itc=False, decim=3, n_jobs=1)
    power.apply_baseline(baseline, mode='mean')
    return (power)
...
def plot_tfr(tfr1, tfr2, picks, bands, tmin=-0.3, tmax=1.0, vmin=-1e-9, vmax=1e-9, cond_1='cond_1', cond_2='cond_2',
             fmin=1.0, fmax=35.0):
    ...
    tfr1.plot(picks=picks, fmin=fmin, fmax=fmax, tmin=tmin, tmax=tmax, vmin=vmin, vmax=vmax, baseline=None, show=False,
              axes=axs[0])
    axs[0].set_title(cond_1)

    tfr2.plot(picks=picks, fmin=fmin, fmax=fmax, tmin=tmin, tmax=tmax, vmin=vmin, vmax=vmax, baseline=None, show=False,
              axes=axs[1])
...
tfr_1 - tfr instance for condition 1
tft_2 - tfr instance for condition 2
bands - str band to plot, ex. 'theta'
cond_1 - str title for condition 1
cond_2 - str title for condition 2
tmin - tmin for plotting in secs (default=0.0)
tmax - tmax for plotting in secs (default=1.0)
threshold - significance threshold (default=0.05)
ms - minimal time window in ms for differences to consider a channel significant

returns tfr plot
'''


def significant_sensors(tfr1, tfr2, bands, cond_1, cond_2, tmin=0, tmax=1, threshold=0.05, ms=200):
    significant_chs = []
    chs = tfr1.ch_names
    ...
    for ch in chs:
        ...
        for i, c in enumerate(clusters):
            c = c[0]
            if ((len(list(range(c.start, c.stop))) >= ms / 15) and (
            any([p <= threshold for p in cluster_p_values]))):  # 3 time samples=approx. 50ms
                sign_ch = ch
...
'''
plots permutation cluster test ERSP for 2 conditions

tfr_1 - tfr instance for condition 1
tft_2 - tfr instance for condition 2
ch - str channel to plot, ex. 'FCz'
cond_1 - str title for condition 1
cond_2 - str title for condition 2
bands - str band to plot, ex. 'theta'
tmin - tmin for plotting in secs (default=-0.3)
tmax - tmax for plotting in secs (default=1.0)
threshold - significance threshold (default=0.05)
...
def plot_sign_channel(tfr1, tfr2, ch, cond_1, cond_2, bands, tmin=-0.3, tmax=1, threshold=0.05):
    if bands == 'delta':
        fmin = 1.0
        fmax = 4.0
    elif bands == 'theta':
    ...
    ax0.plot(times, np.mean(x1, axis=0), label=cond_1, color='red')
    ax0.plot(times, np.mean(x2, axis=0), label=cond_2, color='blue')
    ax0.legend(loc=2)
    ax0.set_ylabel("dB")
    ax0.margins(x=0)
